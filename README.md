...
**Modes:**
- Adding Easy to use jupyter notebook to use it with [colab](https://colab.research.google.com) or similar services.
- Modified [`main_TrainValTest.py`](./main_TrainValTest.py) to run faster.
- Added custom loader script to load my csv datasets, more detail [here](./CSV/README.md).
- Add custom bash scripts to load the datasets
- Add easy [instruction](./README.md#how-to-setup) to setup the enviroment localy using conda.

### How to setup
<!-- to do: fix instruction to be more clear -->

1. Clone the repo
   ```bash
   git clone https://github.com/abdo20050/EEG-ATCNet.git
   ```
...
3. Create environment using conda
   ...
   ```
4. Load the dataset of your choice and don't forget to config the data loader based on your dataset parameters.
   - The configuration will be found in `main_main_TrainValTest.py` or `EEG_ACTNet_notebook.ipynb` under 
    ```python
...
Authors: Hamdi Altaheri, Ghulam Muhammad, Mansour Alsulaiman, Abdelrahman Fatouh

Center of Smart Robotics Research, King Saud University, Saudi Arabia
##
**Updates**: 
* You can test the model using our jupyter notebook [here](./EEG_ACTNet_notebook.ipynb).
* Added CSV file type to load the dataset, see [here](./CSV/README.md) for more instructions.
* Easy installation notes added.
